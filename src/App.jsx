...
const ModelSelectionModal = ({ isOpen, onClose, onSelect, models = [], title }) => {
  ...
  const categorizedModels = useMemo(() => {
    ...
    return { "All": all, "OpenAI": all.filter(m => m.includes('gpt')||m.includes('o1')||m.includes('dall')), "Google": all.filter(m => m.includes('gemini')||m.includes('banana')), "Image": all.filter(m => ['flux','midjourney','banana'].some(k=>m.includes(k))), "Video": all.filter(m => ['kling','luma'].some(k=>m.includes(k))) };
  }, [models, search]);
    ...
    <div className="fixed inset-0 z-[130] bg-black/80 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 w-full max-w-4xl h-[70vh] rounded-2xl flex flex-col overflow-hidden" onClick={e=>e.stopPropagation()}>
        <div className="p-4 border-b border-slate-700 bg-slate-800/50 space-y-4 shrink-0">
          ...
          <input autoFocus value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索..." className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white"/>
        </div>
        <div ref={scrollRef} onWheel={handleWheel} className="px-4 pt-3 border-b border-slate-700 bg-slate-800/30 shrink-0 overflow-x-auto scrollbar-none"><div className="flex gap-2 pb-3 min-w-max">{tabs.map(tab=><button key={tab} onClick={()=>setActiveTab(tab)} className={cn("px-4 py-1.5 text-xs font-medium rounded-full border transition-all", activeTab===tab?"bg-blue-600 border-blue-500 text-white":"bg-slate-800 border-slate-700 text-slate-400")}>{tab}</button>)}</div></div>
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">{categorizedModels[activeTab]?.map(m=><button key={m} onClick={()=>{onSelect(m);onClose()}} className="p-3 rounded border border-slate-800 bg-slate-900 hover:border-blue-500 text-left text-sm text-slate-300 truncate">{m}</button>)}</div>
      </div>
    </div>
  );
};
...
const ModelTrigger = ({ label, icon: Icon, value, onOpenPicker, onManualChange, colorTheme = "slate", className }) => {
  ...
  return (
    <div className={cn("flex items-center rounded-lg border transition-all h-9 group", t.bg, t.border, className || "w-40 md:w-56")}>
      ...
      <button onClick={e=>{e.stopPropagation();setIsManual(!isManual)}} className="px-2 h-full flex items-center justify-center text-slate-500 hover:text-white border-l border-slate-800/50 shrink-0"><Pencil size={12}/></button>
    </div>
  );
};

// C. 全能配置中心 (Fixed: Added "Done" button)
      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-5xl h-[80vh] rounded-2xl shadow-2xl flex overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 flex flex-col bg-slate-900 overflow-y-auto">
          <div className="p-8 border-b border-slate-800 flex justify-between items-center">
              <div><h3 className="text-2xl font-bold text-white flex items-center gap-2">{currentTabInfo.label} 配置</h3></div>
              {/* 修复：加回了完成按钮 */}
          ...
          <div className="p-8 space-y-8">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-800">
              ...
              <div className="space-y-2">
                ...
                <ModelTrigger label="当前模型" icon={currentTabInfo.icon} value={currentConfig.model} onOpenPicker={() => { fetchModels(activeTab); setShowModelPicker(true); }} onManualChange={(v) => updateConfig('model', v)} variant="horizontal" colorTheme={currentTabInfo.color.split('-')[1]} className="w-full" />
...
const InspirationSlotMachine = ({ onClose }) => {
  const { setScript, setDirection } = useProject();
  const [spinning, setSpinning] = useState(false);
  const [result, setResult] = useState(null);
  const slots = { genre: ["赛博朋克", "废土末日", "维多利亚蒸汽", "现代悬疑", "吉卜力", "克苏鲁"], theme: ["时间循环", "AI觉醒", "跨物种友谊", "寻找失落文明", "最后的晚餐", "梦境入侵"], visual: ["霓虹雨夜", "荒漠夕阳", "迷雾伦敦", "极简几何", "水彩手绘", "生物机械"] };
  const spin = () => { setSpinning(true); setResult(null); let i=0; const t=setInterval(()=>{ setResult({genre:slots.genre[Math.floor(Math.random()*6)],theme:slots.theme[Math.floor(Math.random()*6)],visual:slots.visual[Math.floor(Math.random()*6)]}); i++; if(i>15){clearInterval(t);setSpinning(false);} },100); };
  const apply = () => { if(!result)return; setScript(`(开场) 在一个${result.genre}的世界中，故事关于${result.theme}...`); setDirection(`视觉风格：${result.visual}；核心基调：${result.genre}`); onClose(); alert("✨ 灵感已注入！"); };
  return (
    <div className="fixed inset-0 z-[150] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-gradient-to-br from-indigo-900 to-purple-900 border border-purple-500/50 w-full max-w-md rounded-2xl p-8 shadow-2xl text-center" onClick={e=>e.stopPropagation()}>
        <h2 className="text-2xl font-bold text-white mb-6 flex justify-center gap-2"><Sparkles className="text-yellow-400"/> 灵感抽取</h2>
        <div className="grid grid-cols-3 gap-4 mb-8">{["风格","主题","视觉"].map((l,i)=><div key={l} className="bg-black/40 rounded p-4"><div className="text-[10px] text-purple-300 mb-2">{l}</div><div className="text-sm font-bold text-white">{result?Object.values(result)[i]:"-"}</div></div>)}</div>
        <button onClick={spin} disabled={spinning} className="w-full py-4 bg-yellow-500 hover:bg-yellow-400 text-black font-black text-lg rounded-xl mb-4"><Dices size={24}/> {spinning?"...":"摇一摇"}</button>
        {result&&!spinning&&<button onClick={apply} className="w-full py-3 bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm">使用此灵感</button>}
      </div>
    </div>
  );
};
...
// ==========================================
// 模块 2：角色工坊 (CharacterLab - Fixed AR/Angles + Create Actor)
// ==========================================
const CharacterLab = ({ onPreview }) => {
  ...
  // [New] 注册演员功能
  const handleRegisterActor = (name, url) => {
    const newName = prompt("确认演员名称:", name);
    if (newName) {
      setActors(prev => [...prev, { id: Date.now(), name: newName, url: url }]);
      alert(`已将【${newName}】注册为演员！可在自动分镜中调用。`);
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden group hover:border-blue-500/50 transition-all flex flex-col">
        <div className={cn("bg-black relative w-full shrink-0 group/media", arClass)}>
          ...
          : currentImg.url ? (
            <div className="relative w-full h-full group/img cursor-zoom-in" onClick={(e)=>{e.stopPropagation();onPreview(currentImg.url)}}>
              ...
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover/media:opacity-100 transition-opacity"><button onClick={(e)=>{e.stopPropagation(); saveAs(currentImg.url, `view_${index}.png`)}} className="p-1.5 bg-black/60 text-white rounded hover:bg-blue-600"><Download size={12}/></button></div>
              <div className="absolute bottom-2 right-2 flex gap-1 opacity-0 group-hover/media:opacity-100 transition-opacity"><button onClick={handleGen} className="p-1.5 bg-black/60 text-white rounded hover:bg-blue-600"><RefreshCw size={12}/></button></div>
            </div>
          ) : currentImg.error ? (
        ...
        <div className="p-3 border-t border-slate-800 flex-1 flex flex-col min-h-[100px]">
          <div className="flex items-center justify-between mb-2">
            ...
            {/* [New] 注册演员按钮 */}
            {currentImg.url && <button onClick={()=>handleRegisterActor(item.title, currentImg.url)} className="text-[10px] text-green-400 hover:text-green-300 flex items-center gap-1 border border-green-800/50 bg-green-900/20 px-1.5 py-0.5 rounded"><PlusCircle size={10}/> 存为演员</button>}
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-950 relative">
        <div className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-900/30 backdrop-blur-sm z-10"><h2 className="text-slate-400 text-sm hidden md:block">视角预览 ({localPrompts.length})</h2><div className="flex items-center gap-3">{localPrompts.length > 0 && (<><button onClick={() => localPrompts.forEach((p, idx) => handleImageGen(idx, {title:p.title, prompt:p.prompt}, aspectRatio, useImg2Img, referenceImage, imgStrength))} className="flex items-center gap-2 px-3 py-1.5 bg-blue-900/30 hover:bg-blue-800/50 text-blue-200 text-sm rounded border border-blue-800"><Camera size={16}/> 全部生成</button><button onClick={downloadAll} className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm rounded border border-slate-700"><Download size={16}/> 打包下载</button></>)}</div></div>
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-slate-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 pb-20">{localPrompts.map((item, idx) => <CharCard key={idx} item={item} index={idx} currentAr={aspectRatio} currentRef={referenceImage} currentUseImg={useImg2Img} currentStrength={imgStrength}/>)}</div>
        </div>
      </div>
    </div>
  );
...
// ==========================================
// 模块 3：自动分镜工作台 (StoryboardStudio - Fixed UI & UX)
// ==========================================
const StoryboardStudio = ({ onPreview }) => {
  ...
  const handleAnalyzeScript = async () => {
    ...
    try {
      ...
      if (Array.isArray(json)) { setShots(json); setMessages(prev => [...prev, { role: 'assistant', content: `分析完成！设计了 ${json.length} 个镜头。` }]); }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const res = await callApi('analysis', {
        system: "Role: Co-Director. Task: Modify storyboard. Update visual/audio/sora_prompt. Return JSON array ONLY.", 
  ...
  const applyUpdate = () => {
    ...
    updates.forEach(u => { const i = ns.findIndex(s => s.id === u.id); if (i !== -1) ns[i] = { ...ns[i], ...u }; else ns.push(u); });
  ...
  const ChangePreview = () => {
    ...
    return (<div className="bg-slate-800/90 border border-purple-500/50 rounded-lg p-3 my-2 text-xs shadow-lg"><div className="flex justify-between items-center mb-2 pb-2 border-b border-purple-500/20"><span className="font-bold text-purple-300 flex items-center gap-2"><Settings size={12}/> 修改方案 ({updates.length})</span><button onClick={applyUpdate} className="bg-purple-600 hover:bg-purple-500 text-white px-3 py-1 rounded">应用</button></div><div className="space-y-3 max-h-80 overflow-y-auto scrollbar-thin pr-1">{updates.map((u, i) => (<div key={i} className="bg-slate-900/50 p-2.5 rounded border-l-2 border-purple-500"><div className="font-mono text-slate-400 mb-1 font-bold">Shot {u.id}</div><div className="text-slate-300 whitespace-pre-wrap">{u.visual}</div></div>))}</div></div>);
  };
  ...
  const ShotCard = ({ shot, currentAr, currentUseImg, currentAsset, currentStrength }) => {
    ...
    return (
      <div className={cn("bg-slate-900 border border-slate-800 rounded-xl overflow-hidden flex flex-col md:flex-row mb-4 group transition-all hover:border-purple-500/50", selectedShotIds.includes(shot.id) ? "border-orange-500 bg-orange-900/10 ring-1 ring-orange-500" : "")}>
        ...
        <div className="p-4 flex-1 space-y-3 min-w-0 flex flex-col justify-center relative">
          {/* 修复：防误触选择按钮 */}
      ...
      <div className="w-80 flex flex-col border-r border-slate-800 bg-slate-900/50 z-10 shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin">
          ...
          <div className="space-y-2"><label className="text-xs font-bold text-slate-400 flex items-center gap-1.5"><Upload size={12}/> 多模态素材</label><div className="grid grid-cols-3 gap-2 h-20">
              {/* 修复：找回了文字标签 */}
              <div className={cn("relative border border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 transition-colors", mediaAsset?.type==='image'?"border-purple-500 bg-purple-900/20":"border-slate-600")}><input type="file" accept="image/*" onChange={(e)=>handleAssetUpload(e,'image')} className="absolute inset-0 opacity-0 cursor-pointer"/>{mediaAsset?.type==='image' ? <img src={mediaAsset.data} className="w-full h-full object-cover opacity-80"/> : <><ImageIcon size={16} className="mb-1"/><span className="text-[10px]">图片</span></>}</div>
              <div className={cn("relative border border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 transition-colors", mediaAsset?.type==='audio'?"border-purple-500 bg-purple-900/20":"border-slate-600")}><input type="file" accept="audio/*" onChange={(e)=>handleAssetUpload(e,'audio')} className="absolute inset-0 opacity-0 cursor-pointer"/>{mediaAsset?.type==='audio' ? <Mic size={16} className="text-purple-400"/> : <><Mic size={16} className="text-slate-500 mb-1"/><span className="text-[10px]">音频</span></>}</div>
              <div className={cn("relative border border-dashed rounded-lg flex flex-col items-center justify-center cursor-pointer hover:border-purple-500 transition-colors", mediaAsset?.type==='video'?"border-purple-500 bg-purple-900/20":"border-slate-600")}><input type="file" accept="video/*" onChange={(e)=>handleAssetUpload(e,'video')} className="absolute inset-0 opacity-0 cursor-pointer"/>{mediaAsset?.type==='video' ? <Film size={16} className="text-purple-400"/> : <><Film size={16} className="text-slate-500 mb-1"/><span className="text-[10px]">视频</span></>}</div>
          </div></div>
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
          {activeTab === "shots" ? (
            <div className="max-w-4xl mx-auto pb-20 space-y-4">
                ...
                {shots.map(s => <div key={s.id} className={cn("cursor-pointer border-2 rounded-xl transition-all", selectedShotIds.includes(s.id) ? "border-orange-500 bg-orange-900/10 ring-2 ring-orange-500" : "border-transparent")}><ShotCard shot={s} currentAr={sbAspectRatio} currentUseImg={useImg2Img} currentAsset={mediaAsset} currentStrength={imgStrength}/></div>)}
                {shots.length===0 && <div className="text-center text-slate-500 mt-20">暂无分镜</div>}
            </div>
          ) : (
            // [New] 修复：大分镜纵向排列 + 小分镜管理
            <div className="flex flex-col gap-8 max-w-4xl mx-auto pb-20">
                {scenes.map(scene => (
                    <div key={scene.id} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-orange-500/50 transition-all shadow-xl">
                        ...
                        <div className="aspect-video bg-black relative border-b border-slate-800">
                            {scene.video_url ? <video src={scene.video_url} controls className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center relative">{scene.startImg && <><img src={typeof scene.startImg==='string'?scene.startImg:scene.startImg.url} className="w-full h-full object-cover opacity-40"/><div className="absolute inset-0 bg-black/50"/></>}<div className="absolute inset-0 flex items-center justify-center z-10"><button onClick={()=>handleGenSceneVideo(scene)} className="bg-orange-600 hover:bg-orange-500 text-white px-8 py-3 rounded-full font-bold shadow-lg flex items-center gap-2 transform hover:scale-105 transition-all"><Film size={20}/> 生成长视频 ({scene.duration}s)</button></div></div>}
                            <div className="absolute top-4 left-4 bg-orange-600 text-white text-xs px-3 py-1.5 rounded font-bold shadow-lg flex items-center gap-2"><Layers size={14}/> {scene.title}</div>
                        </div>
                        ...
                        <div className="p-6 grid grid-cols-3 gap-6">
                            ...
                            <div className="col-span-2 space-y-3">
                                <div className="flex justify-between items-center"><h4 className="text-xs font-bold text-slate-300">Sora 2 提示词</h4><button onClick={()=>navigator.clipboard.writeText(scene.prompt)} className="text-slate-500 hover:text-white flex items-center gap-1 text-[10px]"><Copy size={12}/> 复制</button></div>
                                <div className="text-xs text-slate-400 font-mono bg-black/30 p-3 rounded-lg max-h-40 overflow-y-auto whitespace-pre-wrap select-all leading-relaxed border border-slate-800/50">{scene.prompt}</div>
                            </div>
                            {/* 右侧：包含的小分镜 */}
                            <div className="space-y-3 border-l border-slate-800 pl-6">
                                <h4 className="text-xs font-bold text-slate-300">包含镜头 ({scene.shots.length})</h4>
                                <div className="space-y-2 max-h-40 overflow-y-auto pr-2 scrollbar-thin">
                                    {scene.shots.map(sid => {
                                        const originalShot = shots.find(s=>s.id===sid);
                                        return (
                                            <div key={sid} className="bg-slate-800/50 p-2 rounded text-[10px] text-slate-400 flex justify-between items-center">
                                                <span className="truncate w-24">Shot {sid}: {originalShot?.visual}</span>
                                                <span className="text-slate-600">{originalShot?.duration}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                ...
                {scenes.length === 0 && <div className="col-span-full text-center text-slate-500 mt-20 flex flex-col items-center gap-2"><Layers size={48} className="opacity-20"/><p>暂无大分镜</p><p className="text-xs text-slate-600">请在“分镜 Shot”标签页选中多个镜头，点击左侧组合按钮。</p></div>}
            </div>
          )}
        </div>
      </div>
