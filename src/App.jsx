...
const ModelSelectionModal = ({ isOpen, onClose, onSelect, models = [], title }) => {
  ...
  const categorizedModels = useMemo(() => {
    ...
    return {
      ...
      "Image": allFiltered.filter(m => ['flux', 'sd', 'stable', 'midjourney', 'mj', 'recraft'].some(k => m.toLowerCase().includes(k))),
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 w-full max-w-4xl h-[70vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
...
// --- 组件：模型触发器 (TopBar Widget) ---
const ModelTrigger = ({ label, icon: Icon, value, onOpenPicker, onManualChange, colorTheme = "slate" }) => {
  const [isManual, setIsManual] = useState(false);
  const themes = { slate: { border: "border-slate-700", icon: "text-slate-400", bg: "bg-slate-900" }, blue: { border: "border-blue-900/50", icon: "text-blue-400", bg: "bg-blue-950/20" }, purple: { border: "border-purple-900/50", icon: "text-purple-400", bg: "bg-purple-950/20" }, orange: { border: "border-orange-900/50", icon: "text-orange-400", bg: "bg-orange-950/20" }, green: { border: "border-green-900/50", icon: "text-green-400", bg: "bg-green-950/20" } };
...
// ==========================================
// 模块：全能配置中心 (Configuration Center)
// ==========================================
const ConfigCenter = ({ config, setConfig, onClose, fetchModels, availableModels, isLoadingModels }) => {
  ...
  const tabs = [
    { id: "analysis", label: "大脑 (LLM)", icon: Brain, desc: "用于剧本分析、提示词生成、导演对话" },
    { id: "image", label: "画师 (Image)", icon: Palette, desc: "用于角色设计、分镜关键帧绘制" },
    { id: "video", label: "摄像 (Video)", icon: Film, desc: "用于最终视频生成 (Sora/Kling/Luma)" },
    { id: "audio", label: "录音 (Audio)", icon: Mic, desc: "用于台词配音、音效生成" },
  ];
      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-5xl h-[80vh] rounded-2xl shadow-2xl flex overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-64 bg-slate-950 border-r border-slate-800 flex flex-col">
          <div className="p-6 border-b border-slate-800">
            ...
            <p className="text-xs text-slate-500 mt-2">独立配置不同能力的 API</p>
          </div>
          <div className="flex-1 py-4 space-y-1 px-2">
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-all", activeTab === t.id ? "bg-blue-900/30 text-white border border-blue-800/50" : "text-slate-400 hover:bg-slate-900 hover:text-slate-200")}>
                ...
                <div><div className="text-sm font-medium">{t.label}</div><div className="text-[10px] opacity-60">设置 {t.id} API</div></div>
              </button>
            ))}
          </div>
          <div className="p-4 border-t border-slate-800 text-center text-xs text-slate-600">Ink & Silk Studio v2.0</div>
        </div>
        ...
        <div className="flex-1 flex flex-col bg-slate-900">
          <div className="p-8 border-b border-slate-800 flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-1">{tabs.find(t => t.id === activeTab).desc}</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8">
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-bold text-slate-300 uppercase tracking-wider">连接设置</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-medium text-slate-400">Base URL (接口地址)</label>
                  <input value={currentConfig.baseUrl} onChange={(e) => updateConfig('baseUrl', e.target.value)} placeholder="https://api.openai.com" className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm text-slate-200 outline-none focus:border-blue-500 font-mono"/>
                  <p className="text-[10px] text-slate-500">例如: https://generativelanguage.googleapis.com (Google)</p>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-medium text-slate-400">API Key (密钥)</label>
                  <input type="password" value={currentConfig.key} onChange={(e) => updateConfig('key', e.target.value)} placeholder="sk-..." className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm text-slate-200 outline-none focus:border-blue-500 font-mono"/>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-800">
              <div className="flex justify-between items-end">
                <h4 className="text-sm font-bold text-slate-300 uppercase tracking-wider">默认模型</h4>
                <button onClick={() => fetchModels(activeTab)} disabled={isLoadingModels} className="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
                  {isLoadingModels ? <Loader2 size={12} className="animate-spin"/> : <RefreshCw size={12}/>} 测试连接并获取列表
                </button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-sm text-slate-200 outline-none focus:border-blue-500 font-mono"
                    placeholder="手动输入或选择..."
                  />
                ...
                <p className="text-[10px] text-slate-500">
                  {activeTab === 'analysis' && "推荐: gemini-1.5-pro, gpt-4o"}
                  {activeTab === 'image' && "推荐: dall-e-3, midjourney, flux"}
                  {activeTab === 'video' && "推荐: luma-dream-machine, kling-v1, runway-gen3"}
                  {activeTab === 'audio' && "推荐: tts-1, eleven-multilingual-v2"}
                </p>
              </div>
            </div>
          </div>
...
const CharacterLab = ({ onGeneratePrompts, onGenerateImage, isGenerating, prompts, images, setAspectRatio, aspectRatio }) => {
  ...
  const [imgStrength, setImgStrength] = useState(0.8); // 默认强一致性
  ...
  return (
    <div className="flex h-full overflow-hidden">
      <div className="w-80 md:w-96 flex flex-col border-r border-slate-800 bg-slate-900/50 p-4 overflow-y-auto z-10 scrollbar-thin scrollbar-thumb-slate-700">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-800/40 p-3 rounded-lg border border-slate-700/50 space-y-4">
             ...
             <div className="pt-2 border-t border-slate-700/50 space-y-2"><div className="flex items-center justify-between"><label className="text-[10px] text-slate-400 flex items-center gap-1"><Sliders size={10}/> 参考图权重</label><input type="checkbox" checked={useImg2Img} onChange={(e) => setUseImg2Img(e.target.checked)} className="accent-blue-600"/></div>{useImg2Img && referenceImage && (<div className="space-y-1 animate-in fade-in"><div className="flex justify-between text-[10px] text-slate-500"><span>Weight: {imgStrength}</span></div><input type="range" min="0.1" max="1.0" step="0.05" value={imgStrength} onChange={(e) => setImgStrength(e.target.value)} className="w-full h-1 bg-slate-700 rounded-lg accent-blue-500 cursor-pointer"/><div className="text-[9px] text-slate-500 leading-tight mt-1">1.0: 强一致 (像原图) - 0.1: 弱一致</div></div>)}</div>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-950 relative">
        ...
        <div className="flex-1 overflow-y-auto p-6"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6 pb-20">{localPrompts.map((item, idx) => <CharCard key={idx} item={item} index={idx} />)}</div></div>
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const [config, setConfig] = useState(() => {
    ...
    return {
      analysis: { baseUrl: oldBase, key: oldKey, model: localStorage.getItem('text_model') || 'gemini-1.5-flash' },
      image: { baseUrl: oldBase, key: oldKey, model: localStorage.getItem('image_model') || 'dall-e-3' },
      video: { baseUrl: '', key: '', model: 'luma-dream-machine' },
  ...
  // --- API: 获取模型列表 (针对特定 Tab) ---
  const fetchModels = async (type = 'analysis') => {
    const { baseUrl, key } = config[type];
    if (!key) return alert(`请先在 [${type}] 设置中填写 API Key`);
    ...
    try {
      ...
      if(found.length) {
        ...
        // 如果是弹窗模式，不需要alert，因为列表会自动更新
        // 如果是配置中心点击测试，给个提示
        if(showSettings) alert(`连接成功！获取到 ${found.length} 个模型。`);
      } else { 
        alert("连接成功，但未获取到模型列表 (可能API不支持列表查询)，请手动输入。"); 
      }
    } catch(e) { alert("连接失败: " + e.message); } 
    finally { setIsLoadingModels(false); }
  };

  // --- API: 核心文本/分析 (路由到 Analysis 配置) ---
  ...
  const callGenerateImage = async (prompt, aspectRatio = "16:9", useImg2Img = false, refImg = null, strength = 0.55) => {
    ...
    // 默认使用 OpenAI 兼容格式 (适用于 DALL-E 3, Flux-Pro, MJ-Proxy)
